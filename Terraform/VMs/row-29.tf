...
resource "aws_instance" "PracticeAnalytics-MES-29" {
  ...
  tags = {
    ...
    Description       = "MicroStrategy Enterprise Server"
   #Name              = "MEDAMPP1MES${format("%02d", count.index + 1)}" #update increment to create machines 4-6
    Name              = "MEDAM${var.Network}${var.Environment-Short}MES${format("%02d", count.index + 4)}" # update increment to create machines 4-6

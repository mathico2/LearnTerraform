...
resource "aws_instance" "TouchWorks-MNGT-14" {
  ...
  ebs_block_device {
   ...
   #Name              = "MEDAMPP1MNGT${format("%02d", count.index + 1)}" #update increment to create machines 4-6
    Name              = "MEDAM${var.Network}${var.Environment-Short}MNGT${format("%02d", count.index + 4)}" # update increment to create machines 4-6

...
resource "aws_instance" "FlexScanMD-API-38" {
  ...
  tags = {
   ...
   #Name              = "MEDAMPP1API${format("%02d", count.index + 1)}" #update increment to create machines 4-6
    Name              = "MEDAM${var.Network}${var.Environment-Short}API${format("%02d", count.index + 4)}" # update increment to create machines 4-6
